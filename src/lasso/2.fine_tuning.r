...
# lasso_fit %>%# lasso_spec <- multinom_reg(mode = "classification", penalty = 0.1, mixture = 1) %>%
#   set_engine("glmnet")
# 
# print(lasso_spec)
# 
...
dt_cv <- vfold_cv(train_set, v=5, repeats = 1, strata = !!config$target_column)
...
lambda_grid <- grid_regular(penalty(), levels = 5, filter = penalty <= .15)
print(lambda_grid)
...
writeLines(" - saving out results from fine-tuning")
fname = file.path(config$prjfolder, config$analysis_folder, "fine_tuned_model.RDS")
saveRDS(object = lasso_grid, file = fname, ascii = FALSE, version = NULL, compress = TRUE, refhook = NULL)
